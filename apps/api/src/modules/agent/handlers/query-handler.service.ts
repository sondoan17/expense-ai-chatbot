...
export class QueryHandlerService {
  ...
  async handleSummary(
    ...
    if (this.shouldUseTemplateReplies()) {
      const fallbackReply = this.buildSummaryFallbackReply(
        payload,
        summary,
        category,
        rangeLabel,
  ...
  private buildSummaryFallbackReply(
    payload: AgentPayload,
    summary: Awaited<ReturnType<TransactionsService['summary']>>,
    ...
    if (payload.intent === 'query_by_category' && category) {
      ...
      return buildSummaryByCategoryReply(language, {
        rangeLabel,
        amountLabel,
    ...
    return buildSummaryTotalsReply(language, {
      rangeLabel,
      expenseLabel,
      incomeLabel,
      netLabel,
